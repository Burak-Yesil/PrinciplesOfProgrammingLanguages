...
and
  ...
  | Proj(e,id) ->
    eval_expr e >>=
    fields_of_recordVal >>=
    fun r -> let point = List.assoc_opt id r in
    (match point with
    | None -> error "not found"
    | Some(true, RefVal va) -> (Store.deref g_store va)
    | Some(false, va) -> (return @@ va)
    | _ -> error "invalid")
  | SetField(e1,id,e2) ->
    eval_expr e1 >>=
    fields_of_recordVal >>=
    fun r -> eval_expr e2 >>=
    fun q -> let point = List.assoc_opt id r in 
    (match point with
    | Some(true, RefVal va) -> (Store.set_ref g_store va q) >>=
      fun f -> return UnitVal
    | Some(false, va) -> error "Field not mutable"
    | _ -> error "invalid")
  | IsNumber(e) ->
    eval_expr e >>= fun va ->
    (match va with
